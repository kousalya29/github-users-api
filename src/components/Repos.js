...
const Repos = () => {
  ...
  let languages =  repos.reduce((total,item)=>{
    const {language, stargazers_count} = item;
      ...
      if(!total[language]) { //first loop
        total[language] = {label:language,value:1, count:stargazers_count};
      } else {
        total[language] = {...total[language], value: (total[language].value +1), count: (total[language].count + stargazers_count)};
      }
